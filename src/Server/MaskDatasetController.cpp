...
MaskDatasetController::MaskDatasetController()
{
    debug_skip_num_ = 1;
}
...
void MaskDatasetController::create_dataset_struct(QString path_to_root)
{
    QDir dir_;
    ...
    {
        if(dir.isDir() && dir.fileName() != "." && dir.fileName() != ".." && !dir.fileName().contains("work_station") && !dir.fileName().contains("train_dataset") && !dir.fileName().contains("valid_dataset"))
        {
...
void MaskDatasetController::get_file_list(QString path_to_root)
{
    QDir dir_;
    ...
    {
        if(file.isDir() && file.fileName() != "." && file.fileName() != ".." && !file.fileName().contains("work_station") && !file.fileName().contains("train_dataset") && !file.fileName().contains("valid_dataset"))
        {
...
void MaskDatasetController::get_absolute_file_path_list()
{
...
void MaskDatasetController::create_work_station()
{
...
void MaskDatasetController::copy_unmarked_dataset()
{
    if(unmarked_dataset_num_ == 0 || workers_num_ < 0)
    {
        return;
        ...
        for(int i = 0; i < workers_num_; ++i)
        {
            ...
            if(file.open(QFile::WriteOnly))
            {
                ...
                for(int j = 0; j < dataset_num_for_each_worker_; ++j)
                {
                    ...
                    {
                        qDebug() << "MaskDatasetController::copy_unmarked_dataset : copy failed.";
...
void MaskDatasetController::init_param(QString root_path, QString target_path, int target_dataset_num, int workers_num)
{
...
void MaskDatasetController::create_mask_work_station()
{
...
void MaskDatasetController::recycle_mask_dataset()
{
    QDir dir_;
    ...
    for(int i = 0; i < workers_num_; ++i)
    {
        ...
        {
            qDebug() << "MaskDatasetController::recycle_mask_dataset : open recycle txt failed.";
            return;
        }
    }
    ...
    for(int i = 0; i < workers_num_; ++i)
    {
        ...
        if(file.open(QFile::ReadWrite))
        {
            while(!file.atEnd())
            {
                ...
                if(dir_.exists(current_work_path + "json/" + str_list[0] + ".json"))
                {
                    ...
                    {
                        qDebug() << "MaskDatasetController::recycle_mask_dataset : copy file failed.";
        ...
        {
            qDebug() << "MaskDatasetController::recycle_mask_dataset : mask idx file open failed.";
            return;
        }
    }
...
void MaskDatasetController::create_train_dataset()
{
    get_absolute_file_path_list();
    ...
    for(QString str : source_file_path_)
    {
        ...
        if(dir_.exists(current_file_info.path() + "/json/" + current_file_info.baseName() + ".json"))
        {
            ...
            {
                qDebug() << "MaskDatasetController::create_train_dataset : copy img failed.";
            ...
            {
                qDebug() << "MaskDatasetController::create_train_dataset : copy json failed.";
...
void MaskDatasetController::create_valid_dataset()
{
    QDir dir_;
...
void MaskDatasetController::recycle_valid_dataset()
{
    QDir dir_;
    ...
    {
        qDebug() << "MaskDatasetController::recycle_valid_dataset : open recycle txt failed.";
        return;
    }
    ...
    if(file.open(QFile::ReadWrite))
    {
        while(!file.atEnd())
        {
            ...
            if(dir_.exists(current_work_path + "json/" + str_list[0] + ".json"))
            {
                ...
                if(!QFile::copy(current_work_path + "json/" + str_list[0] + ".json", file_info.path() + "/json/" + file_info.baseName() + ".json"))
                {
                    qDebug() << "MaskDatasetController::recycle_valid_dataset : copy file failed.";
                    qDebug() << current_work_path + "json/" + str_list[0] + ".json";
                    qDebug() << file_info.path() + "/json/" + file_info.baseName() + ".json";
                    file.close();
                    return;
    ...
    {
        qDebug() << "MaskDatasetController::recycle_valid_dataset : mask idx file open failed.";
        return;
    }
