...
VideoCap::VideoCap(QString ip, QString port, int camera_idx, int cap_time, QString cap_video_path)
{
    ip_ = ip;
    port_ = port;
    ...
    url_path_ = "http://" + ip_ + ":" + port_ + "/stream.mjpg";
    ...
    m_opencvcap = new OpencvCap(this, url_path_);
}
...
void VideoCap::set_param(QString ip, QString port, int camera_idx, int cap_time, QString cap_video_path)
{
    ip_ = ip;
    port_ = port;
    camera_idx_= camera_idx;
...
{
    videowriter_.open((file_title_ + local_index() + ".avi").toStdString(), CAP_FFMPEG, fps_, cv::Size(img_width_, img_height_));
...
QImage VideoCap::receive_mjpg()
{
    ...
    if(m_opencvcap->m_cap.isOpened())
    {
        ...
        {
            qDebug() << "Retrying get_frame ..." << endl;
        }
