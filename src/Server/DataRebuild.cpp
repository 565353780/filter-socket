...
bool DataRebuild::datachange(QString conda_env_name)
{
    ...
    {
#ifdef WIN32
        system(("start conda activate " + conda_env_name + "\npython ../src/Python/datachange.py").toStdString().c_str());
#endif
#ifdef Linux
        system(("RUN_AFTER_BASHRC=\"conda activate " + conda_env_name + "&&python ../src/Python/datachange.py&&exit\" gnome-terminal").toStdString().c_str());
#endif
    }
    else
    {
#ifdef WIN32
        system("start python ../src/Python/datachange.py");
#endif
#ifdef Linux
        system("RUN_AFTER_BASHRC=\"python ../src/Python/datachange.py&&exit\" gnome-terminal");
#endif
    }
...
//    PyObject* pModule = PyImport_ImportModule("datachange");
...
//    PyObject* pFunDataChange= PyObject_GetAttrString(pModule,"Main_DataChange");
...
//    std::cout << "begin datachange" << std::endl;

//    PyObject_CallFunction(pFunDataChange,NULL);

//    std::cout << "end datachange" << std::endl;

    return 1;
}

bool DataRebuild::sample_enhancement(QString conda_env_name, int enhancement_factor)
{
    QString current_path = QDir::currentPath();

    QFile file("../Server_DataBase/message_set/enhancement_factor.txt");

    if(!file.open(QIODevice::WriteOnly))
    {
        qDebug() << "can't open enhancement_factor.txt" << endl;
    }

    file.resize(0);

    file.write(QString::number(enhancement_factor).toStdString().c_str());
    ...
    {
#ifdef WIN32
        system(("start conda activate " + conda_env_name + "\npython ../src/Python/Sample_Enhancement.py").toStdString().c_str());
#endif
#ifdef Linux
        system(("RUN_AFTER_BASHRC=\"conda activate " + conda_env_name + "&&python ../src/Python/Sample_Enhancement.py&&exit\" gnome-terminal").toStdString().c_str());
#endif
    }
    else
    {
#ifdef WIN32
        system("start python ../src/Python/Sample_Enhancement.py");
#endif
#ifdef Linux
        system("RUN_AFTER_BASHRC=\"python ../src/Python/Sample_Enhancement.py&&exit\" gnome-terminal");
#endif
    }
...
//    PyObject* pModule = PyImport_ImportModule("Sample_Enhancement");
...
//    PyObject* pFunSample_Enhancement= PyObject_GetAttrString(pModule,"Main_Sample_Enhancement");

//    if(!pFunSample_Enhancement)
//    {
//        std::cout << "Get function hello failed!\n" << std::endl;
//        return -1;
//    }

//    std::cout << "begin sample enhancement" << std::endl;

//    PyObject_CallFunction(pFunSample_Enhancement,NULL);

//    std::cout << "end sample enhancement" << std::endl;
...
bool DataRebuild::train(QString conda_env_name, QString train_mode)
{
    bool use_tiny = false;
    ...
    QString coco_data_string = dir.absoluteFilePath("../Server_DataBase/train_dataset/darknet_dataset/coco.data");
    QString yolov3_cfg_string = dir.absoluteFilePath("../Server_DataBase/train_dataset/darknet_dataset/yolov3_train.cfg");
    QString weights_string = dir.absoluteFilePath("../pretrained_weights/darknet53.conv.74");
    QString backup_weights_string = dir.absoluteFilePath("../Linux/darknet-master/backup/yolov3_train.backup");

    if(use_tiny)
    {
        yolov3_cfg_string = dir.absoluteFilePath("../Server_DataBase/train_dataset/darknet_dataset/yolov3-tiny_train.cfg");
        backup_weights_string = dir.absoluteFilePath("../Linux/darknet-master/backup/yolov3-tiny_train.backup");
    }

    QFile backup_file(backup_weights_string);

    if(train_mode == "valid")
    {
        yolov3_cfg_string = dir.absoluteFilePath("../Server_DataBase/train_dataset/darknet_dataset/yolov3_test.cfg");
    }

    if(backup_file.exists())
    {
        weights_string = backup_weights_string;
    }

    if(conda_env_name != "")
    {
#ifdef WIN32
        system(("start conda activate " + conda_env_name + "&&E:/chLi/darknet_all_in_one/darknet-master/build/darknet/x64/darknet.exe detector " + train_mode + " " + coco_data_string + " " + yolov3_cfg_string + " " + weights_string + "&&exit").toStdString().c_str());
#endif
#ifdef Linux
    ...
    {
#ifdef WIN32
        system(("start E:/chLi/darknet_all_in_one/darknet-master/build/darknet/x64/darknet.exe detector " + train_mode + " " + coco_data_string + " " + yolov3_cfg_string + " " + weights_string + "").toStdString().c_str());
#endif
#ifdef Linux
        system(("RUN_AFTER_BASHRC=\"" + current_path + "/../Linux/darknet-master/darknet detector " + train_mode + " " + coco_data_string + " " + yolov3_cfg_string + " " + weights_string + "&&exit\" gnome-terminal").toStdString().c_str());
#endif
    }
