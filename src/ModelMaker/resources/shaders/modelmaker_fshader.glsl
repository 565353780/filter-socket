...
void main()
{
    ...
    if(u_selected > 0)
    {
        scolor =  vec3(1.0,0.8,0.2) * 0.7;
    }
    if(u_showtype == 0)
    {
        float val = (unpackInt(v_color));
        float ttype = (mod(val,4.));
        if(abs(ttype - float(Selected)) < 0.1)
        {
            gl_FragColor = vec4(vec3(1.0,0.3,0.3)*lightness,1.0);
        }
        else if(abs(ttype - float(Deleted)) < 0.1 )
        {
            discard;
        }
        else
    ...
    else if(u_showtype == 4)
    {
        ...
        float val = (unpackInt(v_color));
        float ttype = (mod(val,4.));
        if(abs(ttype - float(Deleted)) < 0.1 )
        {
            discard;
            return;
        }
