...
def cost_sensitive(inputs,a=[1.,1.2,1.2,1.2,1.2,1.2,1.2]):
    N = inputs.size(0)
    C = inputs.size(1)
...
if __name__ == "__main__":
    ...
    loadModel ({"model_name":"SandResNet"})
    not_right_count=0
    with  open("./train.json",'r') as f:
        data_list=json.loads(f.read())["train"]
        f.close()
        pass
    for data in data_list:
        result=predict(data)
        if int(result["result"])!=int( data["label"]):
            #print(data["filename"],"not right" )
            not_right_count+=1
    
    print("wrong:",not_right_count,"/total:",len(data_list))
