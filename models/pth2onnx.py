...
class OSNet(nn.Module):
    ...
    def forward(self, x, return_featuremaps=Return_featuremaps):
...
#     map_location = None
# model_url = 'osnet_x0_25_msmt17_combineall_256x128_amsgrad_ep150_stp60_lr0.0015_b64_fb10_softmax_labelsmooth_flip_jitter.pth'
# model.load_state_dict(model_zoo.load_url(model_url, map_location=map_location))

state_dict = torch.load('osnet_x0_25_msmt17_combineall_256x128_amsgrad_ep150_stp60_lr0.0015_b64_fb10_softmax_labelsmooth_flip_jitter.pth')
...
if TestSpeed:
    start = time.time()
    for i in range(1000):
        x = torch.randn(1, 3, 256, 128, requires_grad=False).cuda()
        outputs = model(x)
        if i == 0:
            start = time.time()
            continue
        average_time = 1000 * (time.time() - start) / i
        print('\rAverage spend time : ' + str(int(average_time)) + 'ms, Average fps : ' + str(int((1000 / average_time))), end='')
else:
...
if ChangetoONNX:
    torch.onnx.export(model,
                      x,
                      'osnet_x0_25_msmt17_combineall_256x128_amsgrad_ep150_stp60_lr0.0015_b64_fb10_softmax_labelsmooth_flip_jitter.onnx',
                      export_params=True,
                      opset_version=10,
...
if TestONNX:
    onnx_model = onnx.load('osnet_x0_25_msmt17_combineall_256x128_amsgrad_ep150_stp60_lr0.0015_b64_fb10_softmax_labelsmooth_flip_jitter.onnx')
